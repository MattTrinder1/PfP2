...
namespace API.DataverseAccess
{
    public sealed class DVDataAccess : DVDataAccessBase
    {
        ...
        public DVDataAccess(ConnectionConfiguration config, IMemoryCache cache, Guid contexUserADObjectId)
        {
            ...
            Uri uri = new Uri(config.DVUrl);
            base.dvService = new ServiceClient(uri, config.ClientId, config.ClientSecret, true);
            base.dvService.CallerAADObjectId = contexUserADObjectId;
        }
        ...
        public new Guid GetUserADObjectId(string emailAddress)
        {
            return base.GetUserADObjectId(emailAddress);
        }
